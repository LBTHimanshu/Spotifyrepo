...
class WinampPlayer {
  constructor() {
    this.redirect_uri = "https://web-comps.webflow.io/home-himanshu";
    this.clientID = "230d1b5c150d40728def9abdc2f1313f";
    this.ACCESSTOKEN = "access_token";
    this.spotifyAPIEndpoints = {
  ...
  //   const accessTokenFromLS = this.getAccessTokenFromLS();
  //   console.log({ accessTokenFromLS });
  ...
  updateAccessTokenFromLS(accessToken) {
    console.log({ accessToken });
    // if (!accessToken) return;
    localStorage.setItem(this.ACCESSTOKEN, accessToken);
    console.log(this.getAccessTokenFromLS());
  }
  //fetch playlist

  requestAuthorization() {
    let url = `${this.spotifyAPIEndpoints["AUTHORIZE"]}?client_id=${
      this.clientID
    }&response_type=token&redirect_uri=${encodeURI(
      this.redirect_uri
    )}&show_dialog=true&scope=${encodeURI(
  ...
  getCodeAndUpdateToken() {
    ...
    if (accessToken) {
      this.updateAccessTokenFromLS(accessToken);
    } else {
      this.requestAuthorization();
    }
  }
